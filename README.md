# home-crew-auth-service

Authentication and token issue for customers, workers and admins.
...
|---|---|
| Port | 8082 |
| Route through the gateway | `/auth/**` |
| Java package | `com.homecrew.authservice` |
| Maven artifact | `authservice` |
| Image | `mthanuj/homecrew-auth-service:dev` |
| Compose service | `auth-service` |
| Database | `homecrew_auth` |
| Default branch | `dev` |
...
A push to `dev` builds the image and pushes two tags to Docker Hub:
    ...
    mthanuj/homecrew-auth-service:dev
    mthanuj/homecrew-auth-service:sha-<short>
